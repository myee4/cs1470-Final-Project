...
class ThumbnailModel(tf.keras.Model):
    ...
    def __init__(self, hidden_size, filter_size, embed_size, vocab_size, window_size):
        ...
        self.embed_layer = tf.keras.layers.Embedding(
            input_dim=self.vocab_size, output_dim=self.embed_size, trainable=True,
        ...
        self.images_arch = tf.keras.Sequential(
            layers=[
                tf.keras.layers.Conv2D(self.filter_size, 4, padding="SAME", strides=4,
...
class ImageNumModel(tf.keras.Model):
    ...
    def call(self, images, text, nums):
        ...
        combined_output = tf.concat(
            [images_output, nums_output], axis=1)
        estimation = abs(self.feed_forward(combined_output))
...
class ImageTextModel(tf.keras.Model):
    ...
    def call(self, images, text, nums):
        ...
        combined_output = tf.concat(
            [images_output, text_output], axis=1)
        estimation = abs(self.feed_forward(combined_output))
...
class NumTextModel(tf.keras.Model):
    ...
    def call(self, images, text, nums):
        ...
        combined_output = tf.concat(
            [text_output, nums_output], axis=1)
        estimation = abs(self.feed_forward(combined_output))
